...
class DeepReid(object):
    def __init__(self, cfg, args):
        deepsort = cfg.DEEPSORT
        self.min_confidence = deepsort.MIN_CONFIDENCE
        self.nms_max_overlap = deepsort.NMS_MAX_OVERLAP

        # self.extractor = Extractor(model_path, use_cuda=use_cuda)
        self.extractor = ReidExtractor(args)
        max_cosine_distance = deepsort.MAX_DIST
        nn_budget = 100
        metric = NearestNeighborDistanceMetric("cosine", max_cosine_distance, nn_budget)
        self.tracker = ReIDTracker(Gallery(None), metric, max_iou_distance=deepsort.MAX_IOU_DISTANCE,
